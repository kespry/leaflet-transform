...
exports["default"] = _leaflet2["default"].Class.extend({
  ...
  _projectControlPoints: function _projectControlPoints(points, projection, origin) {
    var projected = points.map(projection).map(function (_ref) {
      var x = _ref.x;
      var y = _ref.y;
      return [x, y];
    });
  ...
  getMercatorTransform: function getMercatorTransform() {
    var projection = _leaflet2["default"].Projection.SphericalMercator.project.bind(_leaflet2["default"].Projection.SphericalMercator);
    return this._getCSSTransformMatrix(this._getTransformMatrix(projection, this._getOrigin(projection)));
  }
});
