...
var _extAffineTransform = require('../../ext/AffineTransform');

var _extLineMarker = require('../../ext/LineMarker');

var _extLineMarker2 = _interopRequireDefault(_extLineMarker);
...
var Path = _SimpleShape2['default'].extend({
	includes: [_extAffineTransform.SetProjections],
	...
	transforms: {
		ui: {
			...
			rotate: function rotate() {
				...
				this._rotateLine = new _extLineMarker2['default'](center, 0, -100, {
					dashArray: [10, 7],
					color: 'black',
					weight: 2
				});
		...
		events: {
			move: function move(newPos) {
				var tx = new _extAffineTransform.Transform(this._map, this.projectionMethods).move(this.getMovePoint(), newPos);
			...
			resize: function resize(latlng) {
				var tx = new _extAffineTransform.Transform(this._map, this.projectionMethods).resize(this._oppositeCorner, this._currentCorner, latlng);
				this._shape.setLatLngs(tx.apply(this._origLatLngs));
			...
			rotate: function rotate(latlng) {
				var tx = new _extAffineTransform.Transform(this._map, this.projectionMethods).rotateFrom(this._origAngle - Math.PI / 2, this._origCenter, latlng);
